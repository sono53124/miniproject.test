...
public class Member extends JPanel {
	...
	String value;// 락커 수정시 중복을 확인해줄 변수
	String mail;// 메일 발송시 수신자를 담을 변수
	boolean overlap;// 등록시 체크방지 담당
...
//      t_key = new JTextField(10);
		...
		edit.addActionListener((e) -> {
			boolean flag = check();
			int updateId = updateMember();
			if (flag) {
				if (updateId == Integer.parseInt(value)) {
		...
		table.addMouseListener(new MouseAdapter() {
			public void mouseReleased(MouseEvent e) {
				...
				value = (String) data.getValueAt(row, 0);
				m_id = value;
